...
if RAW_USER_LOGINS:
    ...
    try:
        ...
        print(f"⚠️ USER_LOGINS direct parse failed: {_e1}")
        # Try 2: extract JSON object (handles leading/trailing garbage)
        try:
            maybe = extract_json_object(RAW_USER_LOGINS)
            if maybe:
                USER_LOGINS = json.loads(maybe)
        except Exception as _e2:
            print(f"⚠️ USER_LOGINS fallback parse failed: {_e2}")
        # Try 3: fix common issues — single quotes, trailing commas
        if not USER_LOGINS:
            try:
                import re as _re

                fixed = RAW_USER_LOGINS
                ...
                print(f"RAW value (first 200 chars): {RAW_USER_LOGINS[:200]}")

print("User login config loaded keys:", list(USER_LOGINS.keys()))

if not USER_LOGINS:
    USER_LOGINS = {
        "Dylan Hicks": {"username": "Dylan", "password": "1234", "sheet": "Dylan Hicks", "role": "coach"},
        "Harrison Hunt": {"username": "Harrison", "password": "4624", "sheet": "Harrison Hunt", "role": "athlete"},
        "Default": {"username": "Default", "password": "1234", "sheet": "Default", "role": "coach"},
        "Cailen Hejka": {"username": "Cailen", "password": "1071", "sheet": "Cailen Hejka", "role": "athlete"},
        "Corey Baker": {"username": "Corey", "password": "1096", "sheet": "Corey Baker", "role": "coach"},
        "Gaby Philis": {"username": "Gaby", "password": "0000", "sheet": "Gaby Philis", "role": "coach"},
        "Alex Dunn": {"username": "Alex", "password": "1444", "sheet": "Alex Dunn", "role": "athlete"},
        "Max Thompson": {"username": "Max", "password": "5371", "sheet": "Max Thompson", "role": "athlete"},
        "Michael Moschou": {"username": "Michael", "password": "1048", "sheet": "Michael Moschou", "role": "athlete"},
    }
...
# Shared compact legend for mobile — single row, abbreviate if needed
_LEGEND_ROW = dict(
...
def wellness_slider(label: str, slider_id: str, value: int = 3):
    """
    A themed 1–5 wellness slider with its label, numeric marks, and an
    always-visible value tooltip. Styling lives in .adptv-slider /
    .adptv-slider-label in assets/dashboard.css.
    """
    return html.Div([
        html.Label(label, className="adptv-slider-label"),
        dcc.Slider(
            id=slider_id, min=1, max=5, step=1, value=value,
            marks=_SLIDER_MARKS_1_5,
            included=True,
            className="adptv-slider",
...
def build_load_plot(df: pd.DataFrame, view_mode: str, theme: str = "light"):
    ...
    if view_mode == "weekly":
        ...
        fig.add_trace(go.Scatter(x=x, y=g["Acute"], name="Acute (4wk)", mode="lines",
                                 line=dict(color=_TEAL, width=2.0, dash="dot"), line_shape="spline",
                                 line_smoothing=0.75,
        ...
        fig.add_trace(go.Scatter(x=x, y=g["ACWR"], name="ACWR", mode="lines", yaxis="y2",
                                 line=dict(color=_PURPLE, width=1.2, dash="dot"), line_shape="spline",
                                 line_smoothing=0.75,
...
def build_speed_tempo_plot(df: pd.DataFrame, view_mode: str, theme: str = "light"):
    ...
    if view_mode == "daily":
        ...
        fig.add_trace(go.Scatter(x=x, y=tempo.ewm(span=7, adjust=False, min_periods=1).mean(),
                                 ...
                                 line=dict(color=_ORANGE, width=1.8, dash="dot"), line_shape="spline",
                                 line_smoothing=0.7,
...
def build_month_calendar(df: pd.DataFrame, month_date: dt.date, selected_date_str: str | None):
    ...
    legend = html.Div([
        ...
        html.Span("1–2", style={"background": "#4285F4", "color": "white", "padding": "2px 8px", "borderRadius": "6px",
                                "marginRight": "6px", "fontSize": "12px"}),
        html.Span("3–5", style={"background": "#4CAF50", "color": "white", "padding": "2px 8px", "borderRadius": "6px",
                                "marginRight": "6px", "fontSize": "12px"}),
        html.Span("6–7", style={"background": "#FF9800", "color": "white", "padding": "2px 8px", "borderRadius": "6px",
                                "marginRight": "6px", "fontSize": "12px"}),
        html.Span("8–10", style={"background": "#F44336", "color": "white", "padding": "2px 8px", "borderRadius": "6px",
                                 "fontSize": "12px"}),
    ...
    <head>
        ...
        <style>
          ...
          @media (min-width: 768px) {
            ...
            /* On desktop hero becomes same size as others */
            .dial-layout-wrap > div:first-child { margin-bottom: 0 !important; }
          }
...
def build_main_layout(auth_data):
    ...
    home_view = html.Div(
        ...
        children=[
            ...
            html.Div(id="wellness-alert-banner", className="mt-2"),
                ...
                children=[
                    ...
                    dbc.Row([
                        ...
                        style={
                            ...
                            wellness_slider("Session RPE (1 = very easy, 5 = maximal)", "slider-session-rpe"),
                            wellness_slider("Session Quality (1 = poor, 5 = excellent)", "slider-session-quality"),
                            wellness_slider("Sleep (1 = tired, 5 = well-rested)", "slider-sleep"),
                            wellness_slider("Mood (1 = sad, 5 = upbeat)", "slider-mood"),
                            wellness_slider("Fatigue (1 = low energy, 5 = energetic)", "slider-fatigue"),
                            wellness_slider("Soreness (1 = low, 5 = high)", "slider-soreness"),
                        ], md=6),
                        dbc.Col([
                            html.Div([
...
def update_welcome(athlete_id, _today):
    ...
    return html.Div([
        ...
        html.Div(sub_line, style={"fontSize": "13px", "color": "var(--text-muted)", "lineHeight": "1.4"}),
...
@app.callback(
    Output("wellness-alert-banner", "children"),
    Input("athlete-dropdown", "value"),
    Input("today-date", "children"),
    prevent_initial_call=True,
)
def update_wellness_banner(athlete_id, _today):
    if not athlete_id:
        raise PreventUpdate
    try:
        df = load_tab_cached(athlete_id)
    ...
    if has_high:
        bg, border, color, icon = "#ffebee", "#ef9a9a", "#C62828", "⚠️"
    else:
        bg, border, color, icon = "#fff8e1", "#ffe082", "#E65100", "•"
    items = []
    for f in flags[:3]:
        items.append(html.Div([
            html.Span(f"{icon} " if f["severity"] == "high" else "• ", style={"fontWeight": "700"}),
            html.Span(f["text"]),
        ], style={"fontSize": "12.5px", "lineHeight": "1.5", "color": color}))
    return html.Div(items, style={
        "background": bg, "border": f"1px solid {border}", "borderRadius": "10px",
        "padding": "10px 14px", "maxWidth": "1000px", "margin": "0 auto",
    })
...
def debug_dates(tab_name):
    if sh is None:
        return jsonify({"error": "no sheet"})
    try:
        ws = sh.worksheet(tab_name)
        # Get all three render options for first 5 rows of Date column
        formula_vals = ws.get_all_values(value_render_option="FORMULA")
        formatted_vals = ws.get_all_values(value_render_option="FORMATTED_VALUE")
        unformatted_vals = ws.get_all_values(value_render_option="UNFORMATTED_VALUE")

        headers = formula_vals[0] if formula_vals else []
        date_idx = headers.index("Date") if "Date" in headers else None

        if date_idx is None:
            return jsonify({"error": "No Date column", "headers": headers[:10]})

        result = {"date_col_index": date_idx, "samples": []}
        for i in range(1, min(6, len(formula_vals))):
            result["samples"].append({
                "row": i + 1,
                "formula": formula_vals[i][date_idx] if i < len(formula_vals) else "N/A",
                "formatted": formatted_vals[i][date_idx] if i < len(formatted_vals) else "N/A",
                "unformatted": unformatted_vals[i][date_idx] if i < len(unformatted_vals) else "N/A",
            })
